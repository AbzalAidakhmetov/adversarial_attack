...
# left off. To re-run a single combo from scratch, delete results/<model>/<attr>/
# first.
#
# Combos (index → label):
#   0  gemma/spanish
#   1  gemma/french
#   2  gemma/has_bold_only
#   3  llama31/lowercase
#   4  llama31/spanish
#
# Submit:    sbatch --array=0-4 slurm/run_best_combo.sh
# Subset:    sbatch --array=0,3 slurm/run_best_combo.sh   # just gemma/spanish + llama31/lowercase
...
case "${SLURM_ARRAY_TASK_ID:-0}" in
    0) EXP="gemma/spanish";        MODEL="google/gemma-2-2b-it";                  ATTR="spanish";        LAYER=14; W=3 ;;
    1) EXP="gemma/french";         MODEL="google/gemma-2-2b-it";                  ATTR="french";         LAYER=14; W=3 ;;
    2) EXP="gemma/has_bold_only";  MODEL="google/gemma-2-2b-it";                  ATTR="has_bold_only";  LAYER=14; W=4 ;;
    3) EXP="llama31/lowercase";    MODEL="meta-llama/Meta-Llama-3.1-8B-Instruct"; ATTR="lowercase";      LAYER=18; W=2 ;;
    4) EXP="llama31/spanish";      MODEL="meta-llama/Meta-Llama-3.1-8B-Instruct"; ATTR="spanish";        LAYER=18; W=3 ;;
...
run_eval() {
    ...
    srun uv run python -m advsteer.eval.evaluate_asr \
