...
def _get_semantic_blacklist() -> set:
    """Load words that should be excluded from the nominally safe vocab."""
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
...
def load_texts_from_json(path: str, n_samples: int) -> List[str]:
    """Load prompt/instruction strings from a JSON list of dicts."""
    with open(path) as f:
        rows = json.load(f)
...
def build_safe_vocab_mask(tokenizer, vocab_size: int, device: str) -> torch.Tensor:
    ...
    This uses a curated positive-only whitelist (~5-10K inert words spanning
    academic, linguistic, geographic, and generic neutral domains).
    ...
    blacklist_words = _get_semantic_blacklist()
    safe_words_lower = {w.lower() for w in safe_words}
    blacklist_words_lower = {w.lower() for w in blacklist_words}
    ...
    for tok_id in range(vocab_size):
        ...
        if word.lower() not in safe_words_lower:
            continue
        if word.lower() in blacklist_words_lower:
    ...
    print(f"Safe vocab mask: {allowed}/{vocab_size} tokens allowed "
          f"({blocked_by_blacklist} blocked by semantic blacklist)")
...
def compute_refusal_direction(
    model, tokenizer, layer_idx: int, harmful_path: str, harmless_path: str,
...
def gumbel_st_optimize(
    ...
    for it in pbar:
        ...
        with torch.no_grad():
            clean_probs = torch.softmax((logits + mask) / max(tau, 0.01), dim=-1)
...
def gcg_optimize(
    ...
    while used_budget < total_budget:
        ...
        if restart == 1:
            ...
            adv_ids = torch.stack([
                allowed_idx[torch.randint(len(allowed_idx), (n_tokens,))]
            ...
            for ki in range(k_total):
                ...
                adv_ids[ki, perturb_pos] = allowed_idx[
...
def optimize_adv(
    ...
    if vocab_mask is not None:
        allowed = allowed & vocab_mask.to(device)
    ...
    print(f"  Gumbel-ST joint score: {gumbel_cos:.4f}")
...
def parse_args():
    ...
    ap.add_argument("--pair_type", type=str, default="emoji", choices=sorted(_PAIR_TYPE_SPECS))
    ...
    ap.add_argument("--refusal_harmful_path", type=str,
                    default="/workspace/adversarial_attack/src/modsteer/dataset/splits/harmful_val.json",
                    help="Held-out harmful prompts used to build the refusal direction.")
    ap.add_argument("--refusal_harmless_path", type=str,
                    default="/workspace/adversarial_attack/src/modsteer/dataset/splits/harmless_val.json",
                    help="Held-out harmless prompts used to build the refusal direction.")
    ...
    ap.add_argument("--refusal_perp", action="store_true",
                    help="Optimize for the component of -refusal ORTHOGONAL to the harmful "
                         "content direction. The harmful direction is computed from a separate "
                         "harmful reference split vs. the dataset benign centroid. Forces the optimizer "
    ...
    ap.add_argument("--harmful_ref_path", type=str, default=None,
                    help="Path to JSON with harmful reference texts for computing harmful_dir "
                         "in --refusal_perp mode. Defaults to harmful_train.json.")
    ap.add_argument("--harmful_ref_samples", type=int, default=64,
                    help="Number of harmful reference prompts to use for harmful_dir.")
    ap.add_argument("--dtype", type=str, default="float32", choices=["float32", "bfloat16"],
                    help="Model dtype. bfloat16 uses ~5GB less VRAM, enabling larger k on "
                         "smaller GPUs. Gumbel logits remain float32 regardless.")
...
def main():
    ...
    refusal_vec = compute_refusal_direction(
        model, tokenizer, hf_layer_idx, args.refusal_harmful_path, args.refusal_harmless_path,
    ...
    if args.refusal_perp:
        harmful_ref_path = (args.harmful_ref_path or
                            "/workspace/adversarial_attack/src/modsteer/dataset/splits/harmful_train.json")
        harmful_ref_texts = load_texts_from_json(harmful_ref_path, args.harmful_ref_samples)

        print(f"\nRefusal-perp mode: computing harmful direction from {len(harmful_ref_texts)} "
              f"reference harmful prompts...")
    ...
    if args.safe_vocab:
        ...
        vocab_mask = build_safe_vocab_mask(tokenizer, vocab_size, device)
